...
export default function Todo({ todo }) {
  ...
  return (
    <div className="todo">
      <h2>{todo.title}</h2>
      <h3 className="todo-completed">Completed: {todo.completed ? "Yes" : "No"}</h3>
      ...
      <button className="btn todo-btn" onClick={complete}>Complete <span>&#10003;</span></button>
      <button className="btn todo-btn" onClick={remove}>Delete <span>&#10005;</span></button>
      {!showEdit && <button className="btn todo-btn" onClick={() => setShowEdit(true)}>Edit <span>&#9998;</span></button>}
      </div>
      {showEdit && (
        <form onSubmit={edit}>
          <input className="edit-input" type="text" name="title" />
          <button className="btn edit-btn" type="submit">OK</button>
        </form>
      )}
    </div>
  );
